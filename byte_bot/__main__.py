...
def get_config() -> Config:
    ...
    role_toggle_channel_id_str = os.environ.get("ByteBotHQ_ROLE_CHANNEL_ID")
    ...
    if not role_toggle_channel_id_str:
        raise ValueError("ByteBotHQ_ROLE_CHANNEL_ID environment variable is not set.")
    ...
    try:
        role_toggle_channel_id = int(role_toggle_channel_id_str)
    except ValueError:
        raise ValueError("ByteBotHQ_ROLE_CHANNEL_ID must be an integer.")
    
    return Config(
        DISCORD_TOKEN=discord_token, FEATURE_FORUM_CHANNEL_ID=forum_channel_id,
        ByteBotHQ_ROLE_CHANNEL_ID=role_toggle_channel_id,
    )
